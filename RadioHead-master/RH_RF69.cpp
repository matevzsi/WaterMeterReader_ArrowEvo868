...
bool RH_RF69::init()
{
    ...
    // Set modem config
    //spiWrite(RH_RF69_REG_02_DATAMODUL, 0x40); // bit sync, no packet handler
    spiWrite(RH_RF69_REG_02_DATAMODUL, 0x00); // bit sync, packet handler
    spiWrite(RH_RF69_REG_03_BITRATEMSB, 0x01); // 100 kHz
    spiWrite(RH_RF69_REG_04_BITRATELSB, 0x40);
    spiWrite(RH_RF69_REG_05_FDEVMSB, 0x03); // 50.11 kHz
    spiWrite(RH_RF69_REG_06_FDEVLSB, 0x35);

    //spiWrite(RH_RF69_REG_18_LNA, 0x08);
    spiWrite(RH_RF69_REG_19_RXBW, 0x09);
    spiWrite(RH_RF69_REG_1A_AFCBW, 0x82);

    //spiWrite(RH_RF69_REG_18_LNA, 0x01); // Max LNA gain with 50 Ohm input
    spiWrite(RH_RF69_REG_18_LNA, 0x00); // Automatic LNA gain control

    spiWrite(RH_RF69_REG_58_TESTLNA, 0x2D); // High sensitivity mode
    spiWrite(RH_RF69_REG_6F_TESTDAGC, 0x30); // Enable DAGC (digital automatic gain control)

    // Packet format
    spiWrite(RH_RF69_REG_2C_PREAMBLEMSB, 0x00); // 
    spiWrite(RH_RF69_REG_2D_PREAMBLELSB, 0x03); // Preamble = 3

    spiWrite(RH_RF69_REG_2E_SYNCCONFIG, 0x88); // Sync on, len = 2
    spiWrite(RH_RF69_REG_2F_SYNCVALUE1, 0x54); // Sync 1 - 0x54 =     01010100
    spiWrite(RH_RF69_REG_2F_SYNCVALUE1 + 1, 0x3D); // Sync 2 - 0x3D = 00111101
    spiWrite(RH_RF69_REG_37_PACKETCONFIG1, 0x08); // Fixed size, no CRC
    spiWrite(RH_RF69_REG_38_PAYLOADLENGTH, 42); // Payload len    28 bytes of data => 42 bytes of 4_to_6 encoded data
    spiWrite(RH_RF69_REG_3D_PACKETCONFIG2, 0x02); // auto restart Rx

    // 868.95 MHz
    /*spiWrite(RH_RF69_REG_07_FRFMSB, 0xD9);
    spiWrite(RH_RF69_REG_08_FRFMID, 0x3C);
    spiWrite(RH_RF69_REG_09_FRFLSB, 0xCD);*/

    // 868.97
    spiWrite(RH_RF69_REG_07_FRFMSB, 0xD9);
    spiWrite(RH_RF69_REG_08_FRFMID, 0x3E);
    spiWrite(RH_RF69_REG_09_FRFLSB, 0x15);


    spiWrite(RH_RF69_REG_01_OPMODE, 0x90); // FSK, no sequencer
...
void RH_RF69::readFifo()
{
    ...
    for (_bufLen = 0; _bufLen < 42; _bufLen++)
